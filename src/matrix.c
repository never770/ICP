...
{
	point_set_matrix *p;
	p = (point_set_matrix *)malloc(sizeof(point_set_matrix));
	p->point_set = AllocateMatrix(row, col);
	return p;
}
void FreePoint_set_matrix(point_set_matrix *p)
{
	FreeMatrix(p->point_set);
	free(p);
}

//将点云集放入到矩阵中，多余的步骤，没有设计好
void PutPointSetIntoMatrix(const point_xyz_set *point_set_A, point_set_matrix * point_set_Matrix)
{
	int point_size = point_set_A->num;
	int i;
	point_set_Matrix->point_set->r = point_size;
	point_set_Matrix->point_set->c = 3;
	for (i = 0; i < point_size; i++)
	{
		(point_set_Matrix->point_set->entry)[i][0] = point_set_A->head[i].x;
		(point_set_Matrix->point_set->entry)[i][1] = point_set_A->head[i].y;
		(point_set_Matrix->point_set->entry)[i][2] = point_set_A->head[i].z;
	}
}

void PutPointIntoMatrix(const point_xyz * A, matrix * pointmatrix)
{
	pointmatrix->entry[0][0] = A->x;
	pointmatrix->entry[0][1] = A->y;
	pointmatrix->entry[0][2] = A->z;
}
...
void initia_matrix4f(Matrix4f center_A, char *type)
{
	...
	char eye[] = "eye";
	//printf("%s   %s\n", type, eye);
	if (strcmpi(type, eye) == 0)
	{
		int i_e;
		for (i_e = 0; i_e <length_matrix; i_e++)
		{
			center_A[i_e][i_e] = 1;
		}
	}
...
void Matrix4fMultPoint_xyz_set(const Matrix4f trans, const point_xyz_set *ps, point_xyz_set *MultResult)
{
	...
	for (i = 0; i < ps->num; i++)
	{
		MultResult->head[i].x = trans[0][0] * (ps->head[i].x) + trans[0][1] * (ps->head[i].y) + trans[0][2] * (ps->head[i].z) + trans[0][3];
		MultResult->head[i].y = trans[1][0] * (ps->head[i].x) + trans[1][1] * ((ps->head[i]).y) + trans[1][2] * ((ps->head[i]).z) + trans[1][3];
		MultResult->head[i].z = trans[2][0] * (ps->head[i].x) + trans[2][1] * ((ps->head[i]).y) + trans[2][2] * ((ps->head[i]).z) + trans[2][3];
	}
}
